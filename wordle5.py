"""
Computer plays wordle (near-optimal guessing).
...
remaining. Words having the form: _a__s or ___es are especially troublesome and
tend to result in lots of guesses when guesses are selected randomly.  So, the
pre-computed word "compt" was found to be a reasonably optimal second guess for
these difficult words. NOTE: compt was optimal for words matching _a_es

For other situations we exhaustively find the optimal guess. There is a
slight tweak to the get_best_guess function in this version so that if
two words produce the same average list size then we show preference to
a guess if it appears in the list of possible answers.
"""
...
def get_best_guess(allwords, possiblewords, wordhash, orig_possible_letters, is_hard_word):
    ...
    for guess in allwords:
        ...
        if total < bestscore or (total == bestscore and guess in possiblewords):
            bestword = guess
            bestscore = total
...
answer = get_random_guess(wordlist)
answer = "antis"
guess = "lares"
result = grade_word(answer, guess, wordhash, possible_letters)
...
while result['grade'] != '22222':
    ...
    is_hard_word = count == 1 and ((result["grade"][1]=='2' and
            result["grade"][4]=='2') or (result["grade"][3] == '2'
            and result["grade"][4] == '2'))
...
print(answer + " solved in " + str(count) + " guesses")
